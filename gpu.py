...
# Configuration
CROSSING_TOLERANCE = 0  # Set to 0 to match test9.py logic
MODEL_CONFIG = {
...
MAX_PROCESSES = 3  # Set to 3 as per user request for handling 3 camera streams
FRAME_SKIP = 5  # Match test9.py
...
VEHICLE_MODEL_PATH = BASE_DIR / "models" / "yolov8x.pt"  # Adjust if needed to match test9.py path
...
def save_screenshot(frame, camera_id, timestamp, prefix):
    ...
    try:
        ...
        print(f"Process {os.getpid()} - Camera {camera_id} - Saved local backup screenshot: {local_filepath}")
        ...
        print(f"Process {os.getpid()} - Camera {camera_id} - Uploaded screenshot to Digital Ocean Space: {cloud_filename}")
        return cloud_filename
    except Exception as e:
        print(f"Process {os.getpid()} - Camera {camera_id} - Failed to save/upload screenshot: {e}")
        return None
    finally:
        gc.collect()

def determine_crossing_vehicle(tracks, a, b, c, frame, camera_id, camera_info, state, shared_state):
    """Determine if vehicle centroids cross the ROI line and classify as Entering/Exiting. Keep logic from test9.py but add dedup/ignore from analytics.py."""
    try:
        ...
        # Get ROI points
        roi_x1, roi_y1 = shared_state["roi_points"][0]
        roi_x2, roi_y2 = shared_state["roi_points"][1]
        roi_p1 = (roi_x1, roi_y1)
        roi_p2 = (roi_x2, roi_y2)
        ...
        for track in tracks:
            ...
            cx, cy = centroid
            print(f"Process {os.getpid()} - Camera {camera_id} - Track {track_id} centroid: ({cx:.1f}, {cy:.1f})")
            ...
            if not track_state['has_crossed']:
                intersects = do_intersect(prev_centroid, curr_centroid, roi_p1, roi_p2)
                # Left to right (entering) - use test9.py logic
            ...
            if abs(curr_dist) > 20:  # Match test9.py reset threshold
...
def select_roi(camera_id, first_frame, width, height, state):
    """Load ROI configuration for a camera from saved JSON file (non-interactive, from analytics.py)."""
        ...
        while cap.isOpened() and not local_stop_event.is_set():
            ...
            for model_id in model_ids:
                ...
                try:
                    ...
                    results = vehicle_model(frame, classes=MODEL_CONFIG[model_id]["classes"], device=device, conf=conf_threshold)
